...
<body>
    ...
    <form action="ChangePrincipalInformation_Show">
    	输入员工号：<input type="text" name="FindNo">
	...
	<table border="1" ><!-- 房记录表  -->
  	...
  	<form id="ADPrincipal" action="ChangePrincipalInformation">
        <input type="button" value="添加员工" onclick="Add()" />
        <input type="hidden" name="OldPno" id="OldPno" />
        <input type="hidden" name="Crud" id="Crud" >
        <input type="button" value="删除员工" onclick="Del()" />
    </form>
	...
	<script type="text/javascript">
		...
		if(c==1){
		 ...
		 function Change(Pno,Pname){
			open ('/SJK/Page/Change/ChangePrincipal.jsp?Pno='+Pno+'&Pname='+Pname, '修改信息' , 'height=400, width=700, top=300, left=500, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=n o, status=no') //这句要写成一行
		 }
		 function Add(){
			open ('/SJK/Page/Change/AddPrincipal.jsp', '增添员工' , 'height=400, width=700, top=300, left=500, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=n o, status=no') //这句要写成一行
		 }
		 function Del(){
			var inPut
